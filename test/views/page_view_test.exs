defmodule Askbywho.PageViewTest do
  ...
  test "#count_emails should get the number of emails on database" do
    %Email{}
      |> Email.changeset(%{"email" => "test@email.com", "name" => "Test", "name_brands" => ["Adidas", "Nike"]})
      |> Repo.insert
  ...
  test "#count_nominations/1 should sum how many nominations were sent for a brand" do
    ...
    %Email{}
      |> Email.changeset(%{"email" => "test2@email.com", "name" => "Test 2", "name_brands" => ["Adidas", "Cavalera"]})
      |> Repo.insert
  ...
  test "#to_go should not show any message of the already got 1k+ of nominations" do
    for n <- 1..1000 do
      %Email{}
        |> Email.changeset(%{"email" => "test-#{n}@email.com", "name" => "Test #{n}", "name_brands" => ["Adidas"]})
        |> Repo.insert
    end
